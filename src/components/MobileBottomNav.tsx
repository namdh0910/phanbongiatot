    ...
    <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-[100] px-2 pb-safe-area-inset-bottom shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
      <div className="flex justify-between items-center h-16 max-w-md mx-auto">
        {navItems.map((item, i) => {
          ...
          const Content = (
            <div className={`flex flex-col items-center justify-center gap-1 w-full min-h-[44px] relative ${isActive ? 'text-[#1b5e20]' : 'text-gray-500'}`}>
          ...
          if (item.isExternal) {
            return (
              <a key={i} href={item.href} target="_blank" className="flex-1 flex items-center justify-center min-h-[44px]">
                {Content}
              </a>
            );
          }
          ...
          return (
            <Link key={i} href={item.href} className="flex-1 flex items-center justify-center min-h-[44px]">
              {Content}
            </Link>
          );
        })}
