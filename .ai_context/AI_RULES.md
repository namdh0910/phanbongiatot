...
- **Primary CTA**: Mọi nút CTA (Call to Action) phải hướng người dùng về **Zalo** hoặc **Hotline** để nhận phác đồ điều trị từ kỹ sư.
...
- **Tư duy phục hồi**: Tuyệt đối không dùng các từ mang tính tiêu cực hoặc độc hại như: *thuốc độc, diệt sạch, hóa chất*.
...
- **Dynamic Routing**: Mọi phác đồ điều trị hoặc chi tiết sản phẩm phải dùng slug tiếng Việt không dấu (VD: `/giai-phap/phuc-hoi-sau-rieng-vang-la`).
- **Data-Driven**: Ưu tiên lưu trữ phác đồ trong `src/data` dưới dạng JSON để dễ dàng quản lý và cập nhật.
